...
exports.extendsSelf = exports.createTypeAdapter = exports.createTypeToImportMapper = exports.createTypesToImportsReducer = exports.getTypeForEnumList = exports.removeTypeModifiers = exports.removeGenerics = exports.normalizeTypeAnnotations = exports.flattenDictionaryTypes = exports.createTypeNormalizer = exports.createTypeParser = exports.createTypeSimplifier = void 0;
...
function createTypeSimplifier() {
    const parseType = createTypeParser(t => {
        let type = t.replace(/(?<![^<, ])System\.([0-9A-Za-z.]+)/g, (_, match) => constants_1.SYSTEM_TYPES.get(match) ?? 'any');
        type = /any</.test(type) ? 'any' : type;
        const { identifier, generics, array } = (0, generics_1.extractSimpleGenerics)(type);
...
function getTypeForEnumList(type) {
    return type.replace(/^.*<([^>]+)>.*$/, '[$1]');
}
exports.getTypeForEnumList = getTypeForEnumList;
...
function createTypeToImportMapper(solution, namespace) {
    ...
    return (type, isEnum) => {
        if (!type || type.startsWith('System'))
            return;
        const modelNamespace = (0, namespace_1.parseNamespace)(solution, type);
...
function createTypeAdapter() {
    const simplifyType = createTypeSimplifier();
    return (type) => (0, tree_1.parseGenerics)(type, node => simplifyType(node.data)).toString();
}
