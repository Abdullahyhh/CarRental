...
exports.isRemoteStreamContentArray = exports.isRemoteStreamContent = exports.createActionToSignatureMapper = exports.createActionToBodyMapper = exports.createActionToMethodMapper = exports.createControllerToServiceMapper = exports.serializeParameters = void 0;
...
function createControllerToServiceMapper({ solution, types, apiName, }) {
    ...
    return (controller) => {
        ...
        imports.push(new models_1.Import({ path: '@abp/ng.core', specifiers: ['RestService', 'Rest'] }));
...
function getTypesWithoutIRemoteStreamContent(types) {
    const newType = { ...types };
    constants_1.VOLO_REMOTE_STREAM_CONTENT.forEach(fileType => {
...
function createActionToBodyMapper() {
    ...
    return ({ httpMethod, parameters, returnValue, url }) => {
        ...
        if (responseType.includes('enum')) {
            ...
            if (responseType === 'enum[]') {
                const normalizedType = (0, type_1.getTypeForEnumList)(type);
...
function createActionToSignatureMapper() {
    ...
    return (action) => {
        ...
        const restConfig = new models_1.Property({ name: 'config', type: 'Partial<Rest.Config>' });
        ...
        signature.parameters = parameters.map(p => {
            ...
            if (isFormData || isFormArray) {
                return new models_1.Property({ name: p.name, type: 'FormData' });
            }
...
function isRemoteStreamContent(type) {
    return constants_1.VOLO_REMOTE_STREAM_CONTENT.some(x => x === type);
}
exports.isRemoteStreamContent = isRemoteStreamContent;
function isRemoteStreamContentArray(type) {
    return constants_1.VOLO_REMOTE_STREAM_CONTENT.map(x => `${x}[]`).some(x => x === type);
}
exports.isRemoteStreamContentArray = isRemoteStreamContentArray;
